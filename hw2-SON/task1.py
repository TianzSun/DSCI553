...
raw = sc.textFile(input_file_path).filter(lambda x: x != "user_id,business_id")

if case_number == "1":
    baskets = raw.map(lambda x: (x.split(',')[0], x.split(',')[1]))\
        .groupByKey().mapValues(set).map(lambda x: x[1])
else:
    baskets = raw.map(lambda x: (x.split(',')[1], x.split(',')[0])) \
        .groupByKey().mapValues(set).map(lambda x: x[1])
...
candidates = baskets.mapPartitions(lambda x: SON_1(x, support, basket_count)).flatMap(lambda x: x)\
    .distinct().collect()
frequent = baskets.flatMap(lambda x: SON_2(x, candidates))\
